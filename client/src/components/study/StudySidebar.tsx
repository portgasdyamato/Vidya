...
function FlashcardList({ flashcards }: { flashcards?: Flashcard[] }) {
  if (!flashcards || flashcards.length === 0) {
    return <p className="text-sm text-white">Flashcards will appear here when available.</p>;
  }
    ...
    <div className="space-y-3">
      {flashcards.map((card, index) => (
        <div key={index} className="rounded-xl border border-border/50 bg-muted/30 p-3 text-sm">
          <div className="flex items-center justify-between text-xs uppercase tracking-wide text-white">
          ...
          {card.options ? (
            ...
            <p className="mt-2 text-white">{card.answer || "No answer provided"}</p>
          )}
        </div>
      ))}
    </div>
    ...
    <div className={`w-full rounded-3xl border border-border/50 bg-card/70 p-4 shadow-lg md:max-w-sm ${collapsed ? "hidden md:block" : "block"}`}>
      <div className="flex items-center justify-between mb-3">
        <div>
          ...
          <p className="text-xs text-white">Create study outputs</p>
        </div>
        <div className="flex items-center gap-2">
          <button
            className="text-sm text-white px-2 py-1 rounded-md hover:bg-muted/10"
      ...
      <div className="space-y-3">
        <div className="rounded-2xl overflow-hidden border border-border/30 bg-gradient-to-r from-card/60 to-background/40 p-3">
          ...
          {flashcards && flashcards.length > 0 ? (
            ...
            <div className="text-xs text-white max-h-36 overflow-hidden">
              <div className="prose prose-sm dark:prose-invert">
                <ReactMarkdown remarkPlugins={[remarkGfm]}>{summary?.substring(0, 600) || ""}</ReactMarkdown>
              </div>
            </div>
          ) : (
            <p className="text-xs text-white">Add a source and generate study outputs to view previews here.</p>
          )}
            <div className="mt-3">
